...
constexpr EventDefinition kCatalog[] = {
...
bool Bit(std::span<const std::uint8_t> bytes, int index) {
  return (bytes[static_cast<std::size_t>(index / 8)] &
          (1U << static_cast<unsigned>(index % 8))) != 0;
}

void SetBit(std::vector<std::uint8_t> &bytes, int index, bool value) {
...
std::string Humanize(std::string_view name) {
  if (StartsWith(name, "EVENT_BEAT_"))
    name.remove_prefix(11);
  else if (StartsWith(name, "EVENT_"))
  ...
  for (const char value : name) {
    ...
    result.push_back(wordStart ? static_cast<char>(std::toupper(ch))
...
void Collect(const json::OrderedJson &records, std::string_view valueKey,
             std::string_view path, std::map<int, bool> &merged,
             std::vector<std::string> &errors) {
  if (!records.is_array()) {
    errors.push_back(std::string(path) + " must be an array");
    return;
  }
  std::set<int> local;
  for (const auto &record : records) {
    try {
      ...
      const auto value = record.at(valueKey).get<bool>();
      if (index < 0 || index >= 2560) {
        errors.push_back(std::string(path) + " has an out-of-range flagIndex");
        continue;
      }
      if (!local.insert(index).second)
        errors.push_back(std::string(path) + " contains a duplicate flagIndex");
      const auto [found, inserted] = merged.emplace(index, value);
      if (!inserted && found->second != value)
        errors.push_back("named event views disagree for flag index " +
                         std::to_string(index));
    } catch (const std::exception &) {
      errors.push_back(std::string(path) +
                       " record requires integer flagIndex and boolean " +
                       std::string(valueKey));
    }
  }
}

std::map<int, bool> Merge(const json::OrderedJson &decoded,
                          std::vector<std::string> &errors) {
  std::map<int, bool> merged;
  if (!decoded.contains("events") || !decoded.at("events").contains("flags") ||
      !decoded.contains("trainerBattles") ||
      !decoded.at("trainerBattles").contains("records") ||
      !decoded.contains("staticBattles") ||
      !decoded.at("staticBattles").contains("records") ||
      !decoded.contains("storyProgress") ||
      !decoded.at("storyProgress").contains("storyFlags")) {
    errors.push_back("named event-state views are incomplete");
    return merged;
  }
  Collect(decoded.at("events").at("flags"), "value", "decoded.events.flags",
          merged, errors);
  Collect(decoded.at("trainerBattles").at("records"), "completed",
          "decoded.trainerBattles.records", merged, errors);
  Collect(decoded.at("staticBattles").at("records"), "completed",
          "decoded.staticBattles.records", merged, errors);
  Collect(decoded.at("storyProgress").at("storyFlags"), "completed",
          "decoded.storyProgress.storyFlags", merged, errors);
  return merged;
}

} // namespace

std::span<const EventDefinition> Catalog() { return kCatalog; }

bool IsStaticEncounter(std::string_view name) {
  return name == "EVENT_BEAT_ARTICUNO" || name == "EVENT_BEAT_ZAPDOS" ||
         name == "EVENT_BEAT_MOLTRES" || name == "EVENT_BEAT_MEWTWO" ||
...
bool IsTrainerBattle(std::string_view name) {
  return StartsWith(name, "EVENT_BEAT_") && !IsStaticEncounter(name);
}

bool IsMajorStory(std::string_view name) {
  static const std::set<std::string_view> names = {
      "EVENT_GOT_STARTER", "EVENT_BATTLED_RIVAL_IN_OAKS_LAB",
      "EVENT_GOT_POKEBALLS_FROM_OAK", "EVENT_GOT_POKEDEX",
      "EVENT_OAK_GOT_PARCEL", "EVENT_GOT_OAKS_PARCEL",
      "EVENT_VIRIDIAN_GYM_OPEN", "EVENT_RESCUED_MR_FUJI",
      "EVENT_GOT_POKE_FLUTE", "EVENT_MET_BILL", "EVENT_GOT_SS_TICKET",
      "EVENT_GOT_HM01", "EVENT_SS_ANNE_LEFT", "EVENT_GOT_MASTER_BALL",
      "EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI",
      "EVENT_ROCKET_DROPPED_LIFT_KEY", "EVENT_BEAT_SILPH_CO_RIVAL",
      "EVENT_BEAT_SILPH_CO_GIOVANNI", "EVENT_GOT_DOME_FOSSIL",
      "EVENT_GOT_HELIX_FOSSIL", "EVENT_BEAT_LANCE",
      "EVENT_BEAT_CHAMPION_RIVAL"};
  return names.contains(name);
}

std::string Category(std::string_view name) {
  if (IsStaticEncounter(name)) return "static_battle";
  if (IsMajorStory(name)) return "major_story";
  if (StartsWith(name, "EVENT_GOT_")) return "item_or_gift";
  if (Contains(name, "BADGE") || Contains(name, "GYM")) return "gym_or_badge";
  if (Contains(name, "UNLOCK") || Contains(name, "LOCK") ||
      Contains(name, "DOOR") || Contains(name, "OPEN"))
    return "door_or_unlock";
  if (Contains(name, "BOULDER") || Contains(name, "SWITCH") ||
      Contains(name, "HOLE"))
    return "environment_puzzle";
  if (Contains(name, "ROCKET") || Contains(name, "SILPH") ||
      Contains(name, "RIVAL") || Contains(name, "FUJI") ||
      Contains(name, "BILL") || Contains(name, "OAK") ||
      Contains(name, "SS_ANNE") || Contains(name, "SAFARI"))
    return "story_progress";
  return "misc_event";
}

std::string Label(std::string_view name) { return Humanize(name); }

json::OrderedJson DecodeNamedState(
    std::span<const std::uint8_t> eventBytes) {
  if (eventBytes.size() != 0x140)
    throw std::runtime_error("event flag range must contain 0x140 bytes");
  json::OrderedJson flags = json::OrderedJson::array();
  json::OrderedJson trainers = json::OrderedJson::array();
  json::OrderedJson statics = json::OrderedJson::array();
  json::OrderedJson stories = json::OrderedJson::array();
  std::size_t set = 0, trainerSet = 0, staticSet = 0, storySet = 0;
  for (const auto &definition : Catalog()) {
    const bool value = Bit(eventBytes, definition.flagIndex);
    set += value;
    const auto label = Label(definition.name);
    const auto category = Category(definition.name);
    flags.push_back({{"flagIndex", definition.flagIndex},
                     {"name", definition.name}, {"label", label},
                     {"category", category}, {"value", value},
                     {"source", "pret/pokered constants/event_constants.asm"},
                     {"confidence", "verified"}});
    if (IsTrainerBattle(definition.name)) {
      trainers.push_back({{"flagIndex", definition.flagIndex},
                          {"name", definition.name}, {"label", label},
                          {"completed", value}});
      trainerSet += value;
    }
    if (IsStaticEncounter(definition.name)) {
      statics.push_back({{"flagIndex", definition.flagIndex},
                         {"name", definition.name}, {"label", label},
                         {"category", "static_battle"},
                         {"completed", value}});
      staticSet += value;
    }
    if (!IsTrainerBattle(definition.name) || IsMajorStory(definition.name)) {
      stories.push_back({{"flagIndex", definition.flagIndex},
                         {"name", definition.name}, {"label", label},
                         {"category", category}, {"completed", value}});
      storySet += value;
    }
  }
  return {{"events", {{"known", flags.size()}, {"set", set}, {"flags", flags}}},
          {"trainerBattles", {{"known", trainers.size()},
                               {"complete", trainerSet}, {"records", trainers}}},
          {"staticBattles", {{"known", statics.size()},
                              {"complete", staticSet}, {"records", statics}}},
          {"storyProgress", {{"known", stories.size()},
                              {"complete", storySet}, {"storyFlags", stories}}}};
}

void ValidateNamedState(const json::OrderedJson &decoded,
                        std::vector<std::string> &errors) {
  const auto merged = Merge(decoded, errors);
  if (!errors.empty()) return;
  const auto expected = Catalog().size();
  if (decoded.at("events").at("flags").size() != expected)
    errors.push_back("decoded.events.flags does not contain the complete verified catalog");
  for (const auto &definition : Catalog()) {
    if (!merged.contains(definition.flagIndex))
      errors.push_back("named event catalog is missing flag index " +
                       std::to_string(definition.flagIndex));
  }
}
...
void ApplyNamedState(const json::OrderedJson &decoded,
  ...
  const auto merged = Merge(decoded, errors);
  if (!errors.empty())
    throw std::runtime_error("invalid named event state: " + errors.front());
  for (const auto &[index, value] : merged)
    SetBit(eventBytes, index, value);
}
