...
OrderedJson Items(const RedSave& save, std::size_t countOffset,
    ...
    for (std::size_t slot = 0; slot < count; ++slot) {
        ...
        if (id == 0xFF) break;
        items.push_back({{"slot", slot + 1}, {"itemId", id},
                         {"quantity", save.At(pairsOffset + slot * 2U + 1)}});
    }
    return {{"declaredCount", declared}, {"count", items.size()},
...
OrderedJson Pokemon(const RedSave& save, std::size_t record,
    ...
    for (std::size_t index = 0; index < 4; ++index) {
        ...
        moves.push_back({{"slot", index + 1}, {"moveId", save.At(record + 0x08 + index)},
                         {"pp", rawPp & 0x3F}, {"ppUps", rawPp >> 6}, {"rawPp", rawPp}});
    }
    OrderedJson result = {
        {"position", position}, {"speciesId", save.At(record)},
        {"level", save.At(record + (party ? 0x21 : 0x03))},
        {"nickname", Text(save, nickname, 11)}, {"otName", Text(save, otName, 11)},
        {"currentHp", c::ReadU16BE(save, record + 0x01)},
        {"status", save.At(record + 0x04)},
        {"types", OrderedJson::array({save.At(record + 0x05), save.At(record + 0x06)})},
        {"catchRate", save.At(record + 0x07)}, {"moves", moves},
        {"trainerId", c::ReadU16BE(save, record + 0x0C)},
        {"experience", c::ReadU24BE(save, record + 0x0E)},
        {"statExperience", {{"hp", c::ReadU16BE(save, record + 0x11)},
        ...
        {"dvs", {{"attack", dv1 >> 4}, {"defense", dv1 & 0x0F},
                 {"speed", dv2 >> 4}, {"special", dv2 & 0x0F},
...
OrderedJson Box(const RedSave& save, std::size_t base, int number) {
    ...
    for (std::size_t index = 0; index < count; ++index) {
        pokemon.push_back(Pokemon(save, base + 0x16 + index * 0x21, 0x21,
            base + 0x2AA + index * 11, base + 0x386 + index * 11, index + 1, false));
    }
    return {{"boxNumber", number}, {"declaredCount", rawCount}, {"count", pokemon.size()},
            {"speciesListHex", c::Hex(save.Slice(base + 1, 20))},
            {"pokemon", pokemon}, {"rawBlockHex", c::Hex(save.Slice(base, 0x462))}};
}
...
OrderedJson Party(const RedSave& save) {
    ...
    for (std::size_t index = 0; index < count; ++index) {
        pokemon.push_back(Pokemon(save, base + 0x08 + index * 0x2C, 0x2C,
            base + 0x110 + index * 11, base + 0x152 + index * 11, index + 1, true));
    }
    return {{"declaredCount", rawCount}, {"count", pokemon.size()},
            {"speciesListHex", c::Hex(save.Slice(base + 1, 6))}, {"pokemon", pokemon}};
}
...
OrderedJson HallOfFame(const RedSave& save) {
    ...
    for (std::size_t entry = 0; entry < count; ++entry) {
        ...
        for (std::size_t slot = 0; slot < 6; ++slot) {
            ...
            pokemon.push_back({{"partyOrder", slot + 1}, {"speciesId", species},
...
OrderedJson Decode(const RedSave& input, const std::string& logicalName,
    ...
    const RedSave::Bytes trailing(all.begin() + static_cast<std::ptrdiff_t>(RedSave::ExpectedSize), all.end());
    ...
    for (std::size_t index = 0; index < 12; ++index)
        boxes.push_back(Box(input, validation::SaveValidator::BoxOffset(index), static_cast<int>(index + 1)));
    ...
    for (std::size_t index = 0; index < report.boxes.size(); ++index)
        boxChecksums.push_back({{"box", index + 1}, {"valid", report.boxes[index].Valid()},
                                {"stored", report.boxes[index].stored},
                                {"calculated", report.boxes[index].expected}});

    OrderedJson badges = OrderedJson::array();
    for (std::size_t bit = 0; bit < 8; ++bit)
        badges.push_back({{"index", bit + 1}, {"owned", (input.At(0x2602) & (1U << bit)) != 0}});

    OrderedJson decoded = {
        {"trainer", {{"name", Text(input, 0x2598, 11)}, {"trainerId", c::ReadU16BE(input, 0x2605)}}},
        {"rival", {{"name", Text(input, 0x25F6, 11)}}},
        {"moneyAndCoins", {{"money", c::ReadBcd(input, 0x25F3, 3)},
                            {"coins", c::ReadBcd(input, 0x2850, 2)}}},
        {"badges", {{"raw", input.At(0x2602)}, {"mirrorRaw", input.At(0x29D6)}, {"entries", badges}}},
        {"playtime", {{"hours", input.At(0x2CED)}, {"maxed", input.At(0x2CEE) != 0},
                      {"minutes", input.At(0x2CEF)}, {"seconds", input.At(0x2CF0)},
                      {"frames", input.At(0x2CF1)}}},
        {"location", {{"mapId", input.At(0x260A)}, {"x", input.At(0x260E)},
                      {"y", input.At(0x260D)}, {"xBlock", input.At(0x2610)},
                      {"yBlock", input.At(0x260F)}, {"previousMapId", input.At(0x2611)}}},
        {"options", {{"raw", input.At(0x2601)}, {"textSpeed", input.At(0x2601) & 0x07},
                     {"battleAnimationsDisabled", (input.At(0x2601) & 0x80) != 0},
                     {"battleStyleSet", (input.At(0x2601) & 0x40) != 0},
                     {"contrast", input.At(0x2609)}}},
        {"pokedex", {{"ownedCount", c::CountSetBits(input, 0x25A3, 19, 151)},
        ...
        {"summaryCounts", {{"eventFlagsSet", c::CountSetBits(input, 0x29F3, 0x140, 0xA00)},
                           ...
                           {"nonzeroScriptBytes", 0}, {"trainerFlagsSet", 0},
                           {"staticEncounterFlagsSet", 0}, {"storyFlagsSet", 0},
                           {"classification", "aggregate counts; named classification deferred"}}}
    };
    std::size_t scripts = 0;
    for (const auto byte : input.Slice(0x289C, 97)) if (byte != 0) ++scripts;
    decoded["summaryCounts"]["nonzeroScriptBytes"] = scripts;

    OrderedJson document = {
        {"schema", {{"format", "pkmn-red-master-save"}, {"schemaVersion", "0.1.0"},
                    {"game", "Pokemon Red"}, {"generation", 1},
                    {"regionAssumption", "USA-Europe"}, {"canonicalExtension", ".red.json"},
                    {"lossless", options.includePhysicalImage}, {"stability", "draft"}}},
        {"tool", {{"name", "pkmn"}, {"version", std::string(kVersion)}}},
        {"source", {{"fileName", logicalName},
                    {"fileSize", {{"decimal", input.Size()}}},
                    {"standardSramSize", {{"decimal", RedSave::ExpectedSize}}},
                    {"trailingByteCount", trailing.size()},
                    {"hashes", {{"wholeFileSha256", wholeHash},
                                {"standardSramSha256", standardHash},
                                {"trailingDataSha256", trailing.empty() ? OrderedJson(nullptr) : OrderedJson(util::Sha256Hex(trailing))}}}}},
        {"integrity", {{"allValid", report.Valid()},
                       {"mainChecksum", {{"valid", report.main.Valid()}, {"storedValue", report.main.stored},
        ...
        {"decoded", decoded},
        {"reconstruction", {{"available", options.includePhysicalImage},
                            {"policy", "use-physical-image-for-no-edit-reconstruction"},
                            {"semanticFieldsAreInformational", true}}},
        {"diagnostics", {{"namedEventClassification", "deferred"},
    ...
    if (options.includePhysicalImage) {
        document["physicalImage"] = {{"encoding", "hex-uppercase-continuous"},
            {"standardSramHex", c::Hex(standard)}, {"trailingDataHex", c::Hex(trailing)},
            {"totalLength", input.Size()}, {"standardSramLength", RedSave::ExpectedSize},
            {"trailingLength", trailing.size()}};
    }
    return document;
}

std::string Serialize(const OrderedJson& document) { return document.dump(2) + "\n"; }
